...
public class CargoCrateTools {
    ...
    @Tool(name = "cargo_crate_search", description = "用于 Rust 项目的 Cargo/crates.io crate 搜索；registryBaseUrl 可选，传入时必须是 Cargo registry API 白名单中的地址。")
    ...
    @Tool(name = "cargo_crate_info", description = "用于 Rust 项目的 Cargo/crates.io crate，查询元数据，包括最新版本、许可证、仓库、关键词、分类和版本摘要。registryBaseUrl 可选，传入时必须是 Cargo registry API 白名单中的地址。")
    ...
    @Tool(name = "cargo_crate_version_detail", description = "用于 Rust 项目的 Cargo/crates.io crate，查询指定版本详情和依赖。version 可选，留空时使用最新稳定版本。rustVersion 表示 crate 声明的最低 Rust 版本或兼容要求，不代表实际编译时使用的 rustc 版本。registryBaseUrl 可选，传入时必须是 Cargo registry API 白名单中的地址。")
